...
from kinbody_detector import DetectorException
import logging

logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)

# Kinbody Detector that uses optimization to make objects lie flat on table. 
# Assumes that table is flat on ground, with known height.

class OptimizingKinBodyDetector(object):
    def __init__(self,
                 env,
                 marker_data_path,
                 kinbody_directory,
    ...
    def optimize(self, kinbody_offset, frame_offset,
        ...
        def cost(x):
            ...
            obj_transform[:, 3] = numpy.array([x[1], x[2], table_height, 1])
            ...
            if marker.ns in self.marker_data:
                ...
                if "table" not in kinbody_file:
                    try:
                        ...
                        obj_transform = self.optimize(kinbody_offset,
                                                         frame_offset,
                                                         marker_pose,
                                                         kinbody_pose,
                                                         table_height)
